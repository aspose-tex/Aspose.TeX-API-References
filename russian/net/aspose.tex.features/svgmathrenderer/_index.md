---
title: SvgMathRenderer
second_title: Справочник по API Aspose.TeX для .NET
description: Реализует рендеринг математической формулы в SVG.
type: docs
weight: 60
url: /ru/net/aspose.tex.features/svgmathrenderer/
---
## SvgMathRenderer class

Реализует рендеринг математической формулы в SVG.

```csharp
public sealed class SvgMathRenderer : MathRenderer
```

## Конструкторы

| Имя | Описание |
| --- | --- |
| [SvgMathRenderer](svgmathrenderer)() | Конструктор по умолчанию. |

## Методы

| Имя | Описание |
| --- | --- |
| [Render](../../aspose.tex.features/mathrenderer/render)(string, Stream, MathRendererOptions, out SizeF) | Визуализирует математическую формулу. |

### Смотрите также

* class [MathRenderer](../mathrenderer)
* пространство имен [Aspose.TeX.Features](../../aspose.tex.features)
* сборка [Aspose.TeX](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.TeX.dll -->
